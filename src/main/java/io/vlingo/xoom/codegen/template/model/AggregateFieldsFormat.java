...
public interface AggregateFieldsFormat<T> {
    ...
    AggregateFieldsFormat<String> NULLABLE_ALTERNATE_REFERENCE = new AlternateReference(field -> "null");
    AggregateFieldsFormat<String> SELF_ALTERNATE_REFERENCE = new AlternateReference(field -> "this." + field);

    default T format(final CodeGenerationParameter aggregate) {
        return format(aggregate, aggregate.retrieveAll(STATE_FIELD));
    }

    T format(final CodeGenerationParameter aggregate, final Stream<CodeGenerationParameter> fields);
        ...
        public List<String> format(final CodeGenerationParameter aggregate,
            ...
            return fields.map(field -> {
                final String fieldType =
                        StateFieldTypeRetriever.retrieve(aggregate, field.value);
    ...
    class AlternateReference implements AggregateFieldsFormat<String> {
        ...
        private final Function<String, String> absenceHandler;

        public AlternateReference(final Function<String, String> absenceHandler) {
            this.absenceHandler = absenceHandler;
        }

        @Override
        public String format(final CodeGenerationParameter aggregate,
                             final Stream<CodeGenerationParameter> fields) {
            ...
            final Function<CodeGenerationParameter, String> mapper = field ->
                    isPresent(field, presentFields) ? field.value : absenceHandler.apply(field.value);

            return aggregate.retrieveAll(STATE_FIELD).map(mapper).collect(Collectors.joining(", "));
        }
